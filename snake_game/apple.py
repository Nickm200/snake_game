import pygame
from settings import *
from random import choice
from math import sin
 
BIOME_IMAGES = {
    "Forest":     join('graphics', 'apple.png'),
    "Desert":     join('graphics', 'flower.png'),
    "Snow":       join('graphics', 'snowflake.png'),
    "Candy Land": join('graphics', 'candy.png'),
    "Sky": join('graphics', 'cloud.png'),
}
 
class Apple:
    def __init__(self, snake, biome):
        self.pos = pygame.Vector2(5,8)
        self.display_surface = pygame.display.get_surface()
        self.snake = snake
        self.biome = biome
        self.set_pos()
 
        self.surf = pygame.image.load(join('graphics', 'apple.png')).convert_alpha()
 
    def update_image(self, random_biome):
        if random_biome:
            path = BIOME_IMAGES.get(self.biome.name, join('graphics', 'apple.png'))
        else:
            path = join('graphics', 'apple.png')
        self.surf = pygame.image.load(path).convert_alpha()
 
    def set_pos(self):
        available_pos = [pygame.Vector2(x, y) for x in range(COLS) for y in range(ROWS)
                         if pygame.Vector2(x, y) not in self.snake.body]
        print(len(available_pos))
        self.pos = choice(available_pos)
 
    def draw(self):
        scale = (1 + sin(pygame.time.get_ticks() / 600) / 3) * 0.13
 
        self.scaled_surf = pygame.transform.smoothscale_by(self.surf, scale)
        self.scaled_rect = self.scaled_surf.get_rect(
            center=(self.pos.x * CELL_SIZE + CELL_SIZE / 2, self.pos.y * CELL_SIZE + CELL_SIZE / 2))
        self.display_surface.blit(self.scaled_surf, self.scaled_rect)
        